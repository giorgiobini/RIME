...
class conv_block(nn.Module):
    def __init__(self,ch_in,ch_out):
        ...
        self.conv = nn.Sequential(
            ...
            SingleValueBN2D(nn.BatchNorm2d(ch_out)),
...
class SingleValueBN2D(nn.Module):
    def __init__(
        self,
        batch_norm2D, *args, **kwargs):
        super().__init__()
        self.eps = batch_norm2D.eps
        self.weight = batch_norm2D.weight
        self.bias = batch_norm2D.bias
        self.running_mean = nn.Parameter(torch.empty(torch.Size([])))
        self.running_var = nn.Parameter(torch.empty(torch.Size([])))
        self.num_batches_tracked = nn.Parameter(torch.empty(torch.Size([]))) #batch_norm2D.num_features
        
    def forward(self, x, *args, **kwargs):
        
        mean = x.flatten(2).mean(dim=-1).unsqueeze(-1).unsqueeze(-1)
        var = x.flatten(2).var(dim=-1).unsqueeze(-1).unsqueeze(-1)
        
        result = (x - mean)/torch.sqrt(var + self.eps)
        result = result * self.weight.reshape(1, -1, 1, 1) + self.bias.reshape(1, -1, 1, 1)
