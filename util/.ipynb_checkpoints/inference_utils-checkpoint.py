...
def associateRIMEpobability(dir):
    
    file_probability_indexed=dir+"predictions.csv"
    
    prob=pd.read_table(file_probability_indexed, sep=",")

    pairs=pd.read_table(dir+"pairs.csv",sep=",",index_col=0)
    ...
    pairs = pairs_prob.copy(deep=True)
