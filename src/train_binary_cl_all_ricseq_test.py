...
def main(args):
    ...
    dataset_val, policies_val = obtain_val_dataset(VAL_DATASET, EASY_PRETRAINING, FINETUNING, args.min_n_groups_val, args.max_n_groups_val, SPECIE, filter_test = False)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # Usage
        merged_dataset = MergedDataset([dataset_splash, dataset_paris])

        # Create DataLoader using the undersampled dataset
        data_loader_train = DataLoader(merged_dataset, batch_size=args.batch_size, shuffle=True, collate_fn=utils.collate_fn_nt2, num_workers=args.num_workers)
