...
import time
sys.path.insert(0, '..')
from util.encoding_sequences import prepare_sequence_for_DNABERT
from config import *
...
from config import ROOT_DIR, processed_files_dir, original_files_dir, rna_rna_files_dir, metadata_dir, embedding_dir, bert_pretrained_dir
...
def main(args):
    ...
    for _, row in df.iterrows():
        ...
        rna1, rna2 = torch.tensor(prepare_sequence_for_DNABERT(row.cdna1), dtype=torch.long), torch.tensor(prepare_sequence_for_DNABERT(row.cdna2), dtype=torch.long)
        rna1, rna2 = torch.unsqueeze(rna1, 0), torch.unsqueeze(rna2, 0)
...
if __name__ == '__main__':
    #run me with: -> 
    #nohup python run_binary_cl_on_test500_BERT.py &> run_binary_cl_on_test500_BERT.out &
    #nohup python run_binary_cl_on_test500_BERT.py --how=val &> run_binary_cl_on_test500_BERT.out &
     
    checkpoint_dir = os.path.join(ROOT_DIR, 'checkpoints_bert', 'binary_cl')
