...
class BinaryClassifierNT1(nn.Module):
...
class BinaryClassifierNT2(nn.Module):
...
def build_arch2(args):
    ...
    model = BinaryClassifierNT2(nt_projection_module, top_classifier, small_cnn, args.use_projection_module)
    
    return model

def build_arch1(args):
    ...
    model = BinaryClassifierNT1(nt_projection_module, small_cnn)
...
def build(args):
    ...
    if args.modelarch == 1:
        model = build_arch1(args)
    elif args.modelarch == 2:
        model = build_arch2(args)
    else:
