...
def main(args):
    ...
    train = df_nt[df_nt.couples.isin(list_train)].drop(['diff1', 'diff2', 'diff', 'problematic'], axis = 1)
   ...
   #-----------------------------------------------------------------------------------------
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_sampled = train_sampled[(train_sampled['diff']<TOLERANCE)&(train_sampled['diff']>-TOLERANCE)].drop(['diff','diff1', 'diff2'], axis = 1)
        train_sampled = balance_df(train_sampled)

        dataset_train = RNADatasetNT(
                gene2info=df_genes_nt,
                interactions=train_sampled,
                subset_file='',
...
if __name__ == '__main__':
    #run me with: -> 
    #nohup python train_binary_cl2_degree.py &> train_binary_cl2_degree.out &

    parser = argparse.ArgumentParser('Training', parents=[get_args_parser()])
    args = parser.parse_args()
    args.output_dir = os.path.join(ROOT_DIR, 'checkpoints', 'binary_cl2_degree')
