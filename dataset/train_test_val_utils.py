...
def sample_gene(list_of_genes, pairs, random_gene = True):
    if random_gene:
        random.shuffle(list_of_genes)
        return list_of_genes[0], list_of_genes[1:]
    else:
        #se ho gia campionato delle coppie, scegliero un gene gia campionato per ridurre l'interconnessione del grafo
        if len(pairs)>0:
            df_test = pd.DataFrame({'couples':pairs})
            regex = df_test['couples'].str.extractall('(.*)_(.*)').reset_index()
            df_test['g1'] = regex[0]
            df_test['g2'] = regex[1]
            df_test_genes = set(df_test['g1']).union(set(df_test['g2']))
            list_to_sample_from = list(set(list_of_genes).intersection(df_test_genes))
            random.shuffle(list_to_sample_from)
            gene_sampled = list_to_sample_from[0]
            return gene_sampled, list(set(list_of_genes) - set(gene_sampled))
        else:
...
def train_test_split_from_df_pairs(df_subset, percentage_training = 0.7, n_randoms=10000000000, seed = 123): 
    ...
    while (interrupt == False):
        gene_sampled, list_of_genes = sample_gene(list_of_genes, gene_pairs_test)
...
def create_or_load_train_test_val(df, save_path):
    ...
    if (os.path.isfile(file_training)) & (os.path.isfile(file_test)) & (os.path.isfile(file_val)):
        ...
        gene_pairs_training, gene_pairs_test, gene_pairs_val = [], [], []
        
        for df in [df_human, df_mouse]:
        
            gp_training_val, gp_test = train_test_split_from_df_pairs(df, percentage_training = 0.85)
            gp_training, gp_val = train_test_split_from_df_pairs(df[df.positive.isin(gp_training_val)], percentage_training = 0.83)
            gp_training, gp_test, gp_val = list(gp_training), list(gp_test), list(gp_val)

            gene_pairs_training += gp_training
            gene_pairs_test += gp_test
            gene_pairs_val += gp_val
            
            # add negative pairs
            gene_pairs_training += list(set(df[df.positive.isin(gp_training)].negative))
