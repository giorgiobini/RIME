...
class Generator_single_input(object):
    def __init__(self, sequence_string, name):
        self.sequence_string = [sequence_string]
        self.name = [name]
        self.load_data()

    def load_data(self):
        self.data_name = np.array([itm.strip()[1:] for itm in self.name])
        self.seq = [itm.strip().upper().replace('T','U') for itm in self.sequence_string]
        self.len = len(self.seq)
        self.seq_length = np.array([len(item) for item in self.seq])
        self.data_x = np.array([self.one_hot_600(item) for item in self.seq])
        self.seq_max_len = 600
    ...
    def one_hot_600(self,seq_item):
        ...
        if len(seq_item) <= 600:
            ...
            #one_hot_matrix_600 = np.zeros((600,4)) #before this was not commented and the one below was commented
            one_hot_matrix_600 = np.zeros((len(seq_item),4))
        one_hot_matrix_600[:len(seq_item),] = feat
        return one_hot_matrix_600
...
class Generator_multiple_inputs(object):
    #added by me
    def __init__(self, sequence_strings, names):
        self.sequence_strings = sequence_strings
        self.names = names
        self.load_data()

    def load_data(self):
        self.data_name = np.array(self.names)
        self.seq = [itm.strip().upper().replace('T','U') for itm in self.sequence_strings if itm.upper().startswith(('A','U','C','G','T'))]
        self.len = len(self.seq)
        self.seq_length = np.array([len(item) for item in self.seq])
        self.data_x = np.array([self.one_hot_600(item) for item in self.seq])
        self.seq_max_len = 600
